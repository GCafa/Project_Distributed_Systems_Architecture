...
Espone due endpoint TCP:
- endpoint client read-only, sulla porta configurata;
- endpoint interno di replica, sulla porta configurata + 100.

Il protocollo client e' testuale. Il protocollo di replica e' JSON per riga.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Secondary read-only per KV store")
...
class SecondaryStore:
    """
    Replica secondaria read-only per i client.

    Le mutazioni arrivano solo dal canale interno di replica. Il delay opzionale
    serve a rendere osservabile una lettura stantia durante gli esperimenti.
    """
    ...
    def execute_client(self, line: str) -> tuple[str, bool]:
        """Esegue un comando client read-only."""
        stripped = line.strip()
        if not stripped:
    ...
    def apply_replication(self, record: dict[str, str]) -> str:
        """Applica un record di replica proveniente dal primary."""
...
def handle_client_connection(
    ...
    """Gestisce una connessione client read-only."""
...
def handle_replica_connection(
    ...
    """Gestisce una connessione interna di replica."""
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as replica_socket:
            ...
            def accept_loop(
                ...
                handler: Callable[
                    [socket.socket, tuple[str, int], SecondaryStore],
                    None,
                ],
            ) -> None:
                while True:
