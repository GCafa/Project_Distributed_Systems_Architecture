...
Semantica:
- il primary applica la scrittura localmente;
- risponde subito OK al client;
- poi tenta di replicare l'update verso il secondario in un thread daemon.

Di conseguenza, una lettura immediata dal secondario puo' essere stantia.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Primary asincrono per KV store")
...
class AsyncPrimaryStore:
    """
    KV store primary con replica asincrona verso un solo secondario.

    _data e' lo stato autorevole locale del primary. Un lock protegge accessi
    concorrenti da piu' client.
    """
    ...
    def _replicate_async(self, record: dict[str, str]) -> None:
        """Invia un update al secondario in background."""
        threading.Thread(
    ...
    def _send_replication(self, record: dict[str, str]) -> None:
        """Protocollo interno: JSON newline-delimited verso il secondario."""
        try:
...
def serve() -> None:
    """Entry point del primary asincrono."""
    args = parse_args()
