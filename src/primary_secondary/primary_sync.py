...
Semantica:
- per ogni scrittura, il primary invia prima l'update al secondario;
- risponde OK solo se il secondario conferma con ACK;
- se il secondario non e' raggiungibile, la scrittura fallisce.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Primary sincrono per KV store")
...
class SyncPrimaryStore:
    """
    KV store primary con replica sincrona verso un solo secondario.

    Una write e' visibile localmente solo dopo ACK della replica. Questo rende
    piu' forte la semantica di commit, ma riduce la liveness se la replica cade.
    """
    ...
    def execute(self, line: str) -> tuple[str, bool]:
        """Esegue un comando testuale e restituisce (risposta, should_close)."""
        stripped = line.strip()
        if not stripped:
    ...
    def _replicate_and_ack(self, record: dict[str, str]) -> tuple[bool, str]:
        """Invia l'update al secondario e richiede ACK prima di proseguire."""
        try:
            with socket.create_connection(
                (self._secondary_host, self._secondary_port), timeout=1.0
            ) as connection:
    ...
    def _handle_set(self, argument_blob: str) -> tuple[str, bool]:
        ...
        key, value = parts
        with self._lock:
            replicated, message = self._replicate_and_ack(
...
def serve() -> None:
    """Entry point del primary sincrono."""
    args = parse_args()
